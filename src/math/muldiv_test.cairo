...
fn test_muldiv_div_by_zero() {
    let x = muldiv(u256 { low: 0, high: 1 }, u256 { low: 0, high: 1 }, u256 { low: 0, high: 0 });
}
...
fn test_muldiv_up_div_by_zero_no_overflow() {
    let x = muldiv(u256 { low: 0, high: 1 }, u256 { low: 1, high: 0 }, u256 { low: 0, high: 0 });
}
...
fn test_muldiv_overflows() {
    ...
    muldiv(u256 { low: 0, high: 1 }, u256 { low: 0, high: 1 }, u256 { low: 1, high: 0 });
}

#[test]
fn test_muldiv_fits() {
    // 2**128 * 2**128 / 2 = 2**256 / 2 = 2**255
    let x = muldiv(u256 { low: 0, high: 1 }, u256 { low: 0, high: 1 }, u256 { low: 2, high: 0 });
...
fn test_muldiv_up_fits_no_rounding() {
    // 2**128 * 2**128 / 2 = 2**256 / 2 = 2**255
    let x = muldiv_up(u256 { low: 0, high: 1 }, u256 { low: 0, high: 1 }, u256 { low: 2, high: 0 });
    assert(x == u256 { low: 0, high: 0x80000000000000000000000000000000 }, 'result');
}
...
fn test_muldiv_phantom_overflow() {
    let x = muldiv(u256 { low: 0, high: 5 }, u256 { low: 0, high: 10 }, u256 { low: 0, high: 2 });
...
fn test_muldiv_up_phantom_overflow_no_rounding() {
    let x = muldiv_up(
        u256 { low: 0, high: 5 }, u256 { low: 0, high: 10 }, u256 { low: 0, high: 2 }
    );
    assert(x == u256 { low: 0, high: 25 }, 'result');
}
...
fn test_muldiv_up_no_overflow_rounding_min() {
    let x = muldiv_up(u256 { low: 1, high: 0 }, u256 { low: 1, high: 0 }, u256 { low: 2, high: 0 });
