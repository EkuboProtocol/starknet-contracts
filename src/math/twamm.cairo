...
// Computes the sale rate as a fixed point 96.32 number for a given amount, which is the just the
// amount divided by the duration in seconds The maximum sale rate for a given 18 decimal token that
// can be expressed in a 96.32 is approximately ~79,228,162,514 tokens
...
// Computes the amount for the given sale rate over the given start and end time, which is just the
// amount times the duration Will never revert because we limit the duration of any order to 2**32
...
pub fn calculate_next_sqrt_ratio(
    ...
    let sqrt_sale_ratio: u256 = if (sale_ratio.high.is_zero()) {
        (u256 { high: sale_ratio.low, low: 0 }).sqrt().into()
    } else {
        (sale_ratio.sqrt()).into() * constants::X64_u256
    };
    ...
    let sqrt_ratio_next = if (c.is_zero() || liquidity.is_zero()) {
        ...
        // calculate e
        // sqrt_sale_rate * 2 * t
        let result_mul: u256 = sqrt_sale_rate.into() * (0x200000000 * time_elapsed.into());
