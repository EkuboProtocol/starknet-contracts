...
// Compute x * y / z without overflowing if the result fits within 256 bits
fn muldiv(x: u256, y: u256, z: u256) -> u256 {
    let numerator = u256_wide_mul(x, y);

    if ((numerator.limb3 == 0) & (numerator.limb2 == 0)) {
        return u256 { low: numerator.limb0, high: numerator.limb1 } / z;
    }

    assert(z > u256 { low: numerator.limb2, high: numerator.limb3 }, 'MULDIV_OVERFLOW_OR_DBZ');

    let (res, _) = u512_safe_div_rem_by_u256(numerator, u256_as_non_zero(z));
    return u256 { low: res.limb0, high: res.limb1 };
}

// Compute ceil(x * y / z) without overflowing if the result fits within 256 bits
fn muldiv_up(x: u256, y: u256, z: u256) -> u256 {
    let numerator = u256_wide_mul(x, y);
