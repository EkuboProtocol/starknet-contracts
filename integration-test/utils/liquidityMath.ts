...
function tickToSqrtRatio(tickBig: bigint): bigint {
  if (tickBig < MIN_TICK || tickBig > MAX_TICK)
    throw new Error(`Invalid tick: ${tickBig}`);
  let sign = tickBig < 0n;
  let tick = Math.abs(Number(tickBig));
